...
- **`VibeVoiceEngine`** — реализация через oMLX HTTP API, автоматическое разбиение длинного аудио по тишине (librosa), определение спикеров (ID из ответа oMLX), парсинг JSON сегментов (прямой массив, конкатенированные oMLX объекты, code blocks, regex fallback)
