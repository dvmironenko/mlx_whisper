...
LANGUAGE=ru    # Язык по умолчанию для транскрипции (empty = auto-detect). Поддерживаемые: ru, en, de, fr, es, ...
...
# Порог отсутствия речи (по умолчанию: 0.6). Если вероятность того, что в сегменте "нет речи", выше этого порога, а средняя логарифмическая вероятность (logprob) ниже logprob_threshold, сегмент считается тишиной

HALLUCINATION_SILENCE_THRESHOLD=2.0  

# Порог галлюцинаций/тишины (по умолчанию: 2.0 сек.) Этот параметр помогает бороться с "галлюцинациями" на длинных паузах. Если модель генерирует текст в сегменте, где уровень громкости ниже определенного порога в течение этого времени, Whisper может проигнорировать этот вывод

# --- Initial Prompt (Начальный промпт) ---

INITIAL_PROMPT=Транскрибируй аудио файл.

# Начальный текст для контекстной транскрипции (опционально)

# --- Default Model (Модель по умолчанию) ---

MODEL=turbo

# Модель по умолчанию: tiny, base, small, medium, turbo, large

# --- Default Task (Задача по умолчанию) ---

TASK=transcribe

# Тип задачи по умолчанию: transcribe или translate

# --- Word Timestamps (Временные метки) ---

WORD_TIMESTAMPS=false

# Включить word-level timestamps по умолчанию

# --- Condition on Previous Text (Контекст) ---

CONDITION_ON_PREVIOUS=true

# Использовать предыдущий текст для контекста по умолчанию
