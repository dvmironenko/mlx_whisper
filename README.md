...
DEFAULT_LANGUAGE=ru               # Язык по умолчанию для транскрипции (empty = auto-detect)
...
# ========================================
# Default Transcription Settings - Настройки по умолчанию
# ========================================

DEFAULT_TASK=transcribe           # Тип задачи по умолчанию: transcribe или translate
DEFAULT_MODEL=large               # Модель по умолчанию: tiny, base, small, medium, turbo, large
DEFAULT_WORD_TIMESTAMPS=false     # Включить word-level timestamps по умолчанию
DEFAULT_CONDITION_ON_PREVIOUS=true  # Использовать предыдущий текст для контекста по умолчанию
```
...
- `language` - Язык аудио (empty = auto-detect). Поддерживаемые: ru, en, de, fr, es и др.
- `task` - Тип задачи: `transcribe` (транскрипция) или `translate` (перевод на английский)
- `model` - Размер модели: tiny, base, small, medium, turbo, large
- `word_timestamps` - Включение временных меток для слов (true/false)
- `condition_on_previous_text` - Использование предыдущего текста для контекста (true/false)
