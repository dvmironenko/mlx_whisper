...
- Поддержка светлой/тёмной темы с переключателем
- Drag & Drop загрузка файлов
- Аккордеон для параметров транскрипции
...
### REST API Endpoints
- `/` - веб-интерфейс, предоставляющий HTML шаблон (`new_index.html`)
- `/api/v1/health` - проверка состояния сервиса
- `/api/v1/config` - текущая конфигурация из `.env`
- `/api/v1/models` - список поддерживаемых моделей
- `/api/v1/jobs/{job_id}` - проверка статуса задачи транскрипции
- `/api/v1/transcribe` - основная точка входа для транскрибации аудиофайлов

### Особенности веб-интерфейса
- **Drag & Drop** загрузка аудиофайлов с поддержкой всех форматов
- **Тёмная/светлая тема** с сохранением выбора в localStorage
- **Сворачиваемая секция** параметров транскрипции (аккордеон)
...
### Основные эндпоинты:
1. `/` - Возвращает веб-интерфейс (`new_index.html`)
2. `/api/v1/health` - Проверяет состояние сервиса
3. `/api/v1/config` - Возвращает текущую конфигурацию из `.env`
4. `/api/v1/models` - Возвращает список поддерживаемых моделей
5. `/api/v1/jobs/{job_id}` - Проверяет статус задачи транскрипции
6. `/api/v1/transcribe` - Основная точка входа для транскрипции аудиофайлов
