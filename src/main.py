...
executor = ThreadPoolExecutor(
    max_workers=2
)  # Reduced to 2 workers for better resource management
...
def transcribe_audio_sync(
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            text = (
                str(sanitized_result.get("text", ""))
                if "text" in sanitized_result and sanitized_result["text"] is not None
                else ""
            )
        ...
        job_status[job_id]["duration"] = (
            job_status[job_id]["end_time"] - job_status[job_id]["start_time"]
        )
...
async def process_transcription(
    ...
    if file.size is not None and file.size > MAX_FILE_SIZE:
        raise HTTPException(
            status_code=413, detail="File too large. Maximum file size is 5GB"
        )
    ...
    if task not in ["transcribe", "translate"]:
        raise HTTPException(
            status_code=400, detail="Task must be either 'transcribe' or 'translate'"
        )
    ...
    try:
        ...
        if not convert_to_wav(tmp_file_path, converted_file_path):
            raise HTTPException(
                status_code=500, detail="Failed to convert audio file to WAV format"
            )

        # Log the transcription request
        logger.info(
            f"Transcribing {converted_filename} with model {model}, language {language}"
        )
...
async def transcribe_audio(
    ...
    try:
        ...
        no_speech_threshold_float = (
            float(no_speech_threshold) if no_speech_threshold is not None else 0.4
        )
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid parameter values: {str(e)}"
        )
